...
<div class="main-card mb-3 card">
    <div class="card-body">
        ...
        <div class="collapse" id="collapseExample123" style="">
            <form method="post" action="marque/create">
                {{ csrf_field() }}
...
<table id="myTable" class="mb-0 table table-striped" style="overflow: scroll">
    ...
    <tbody>
        @foreach($marques as $m)
            ...
            <td>{{ $m->id}}</td>
            <td>{{ $m->name}}</td>
            <td><img style="max-height: 100px" src=" {{ $m->logo}} " />  </td>
            <td> {{ $m->created_at }}</td>
            <td> {{ $m->updated_at }}</td>
           ...
           <a  href={{"marque/edit/$m->id"}} title='Editer' data-toggle='tooltip'><span class='pe-7s-pen'></span></a>
           </td>
        </tr>
